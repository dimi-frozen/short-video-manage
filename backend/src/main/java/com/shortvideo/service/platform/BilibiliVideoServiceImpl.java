...
@Service
public class BilibiliVideoServiceImpl implements VideoPlatformService {

    private final VideoInfoService videoInfoService;
    private final StringRedisTemplate stringRedisTemplate;
    private final PlatformTokenStore platformTokenStore;

    @Value("${platform.bilibili.token-ttl-seconds:7200}")
    private long tokenTtlSeconds;

    @Value("${platform.bilibili.fixed-access-token:bilibili_mock_access_token}")
    private String fixedAccessToken;

    @Value("${platform.bilibili.upload-base-dir:${user.home}/short-video/upload/bilibili}")
    private String uploadBaseDir;

    public BilibiliVideoServiceImpl(VideoInfoService videoInfoService, StringRedisTemplate stringRedisTemplate, PlatformTokenStore platformTokenStore) {
    ...
    public AuthUrlResp getAuthUrl() {
        ...
        try {
            platformTokenStore.put("bilibili", token, tokenTtlSeconds);
        } catch (Exception ignored) {
        }

        AuthUrlResp resp = new AuthUrlResp();
        resp.setAuthUrl("https://open.bilibili.com/oauth2/authorize?client_id=mock&response_type=code&scope=user_info&redirect_uri=http://localhost/mock");
    ...
    public PublishResp publish(MultipartFile file, String title, String accessToken) {
        ...
        String videoId = "bl_" + UUID.randomUUID().toString().replace("-", "");
        ...
        info.setPlatformType(PlatformType.BILIBILI.name());
        info.setViews(0L);
        info.setLikes(0L);
        info.setComments(0L);
        info.setShares(0L);
    ...
    public PublishResp publishByUrl(String fileUrl, Long fileSize, String title, String accessToken) {
        ...
        String videoId = "BV" + core.substring(0, 10);
        LocalDateTime now = LocalDateTime.now();
    ...
    public VideoStatsResp stats(String videoId) {
        ...
        long incViews = 40 + r.nextInt(2200);
        long incLikes = r.nextInt(320);
        long incComments = r.nextInt(90);
        long incShares = r.nextInt(35);
    ...
    public IPage<VideoListItemResp> page(int pageNum, int pageSize, String sortBy, String sortOrder, LocalDateTime startTime, LocalDateTime endTime) {
        ...
        qw.eq(VideoInfo::getPlatformType, PlatformType.BILIBILI.name());
        if (startTime != null) {
    ...
    private void validateToken(String token) {
        ...
        if (cached == null) {
            ...
            try {
                info = platformTokenStore.get("bilibili");
            } catch (Exception ignored) {
            }
    ...
    private VideoInfo requireVideo(String videoId) {
        VideoInfo info = videoInfoService.getOne(new LambdaQueryWrapper<VideoInfo>()
                ...
                .eq(VideoInfo::getPlatformType, PlatformType.BILIBILI.name()));
        if (info == null) {
